...
- **Automation health** is a projection on the case itself: current automation state, exact blocker, next action, retry owner and time, and the timestamp of the last material progress. It is not a second lifecycle or run tracker. Boundary proof producers separately emit content-free run receipts so the projection can prove that its retry owner is alive.
...
| Proof-producer health | `release_promotion`, `post_deploy_private_replay`, or `production_observation` | A later successful run receipt from that exact producer |
