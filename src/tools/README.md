...
- `agentModelIntent.ts`, `agentModelCatalog.ts`, and `agentModelTools.ts`: owner/ops-only durable per-server NanoCodex switching and reset. Explicit natural requests accept only Sol, Luna, or their exact OpenRouter IDs. The current message remains authoritative when a model-generated argument conflicts, and successful changes apply before remaining same-request work and to future requests.
