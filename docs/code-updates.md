...
For work linked to an improvement case, task success means only that the case is ready for deployed verification. The private eval stage records content-free per-contract results, durable promotion applies any complete receipt, and production observation later supplies traffic-sampled quality proof. Passed receipts resolve the case, failed receipts return it to `actionable`, and missing runtime/delivery proof leaves it `verifying`. Readiness or an operator-authored summary never substitutes for the active contract.
