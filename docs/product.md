...
- A member can report an assistant response by adding `🐛` to that bot-authored message; reactions on member-authored messages do not create reports. A `🐛` report is triaged silently. When an exact reporter answer is required, the bot mentions that reporter in a reply to the original report; otherwise investigation and repair stay silent. The only later member-facing update is the final reply after the related fix has been deployed and verified. Improvement delivery never creates a thread or sends a DM.
