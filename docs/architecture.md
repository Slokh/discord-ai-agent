...
6. Sandbox callbacks hit `src/control/internalApi.ts`, which persists command events, artifacts, spans, and terminal output. Worker lifecycle state such as start, progress, and completion is recorded through repository lifecycle methods that fan out to both legacy codegen events and `agent.task.*` runtime events.
