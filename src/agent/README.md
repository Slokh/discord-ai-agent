...
- `randomOutcomeGuard.ts`: detects fresh chance outcomes that lack a successful `drawRandom` result, including ordinary short random-number answers, treats a rejected/invalid draw attempt as incomplete even when the draft avoids stating an outcome, drives one in-turn retry, and provides the fail-closed response used by the model loop. Retry routing forces a draw only when the immutable current request explicitly starts or clearly continues a scoped chance action; reply context can identify that continuation but cannot authorize randomness by itself.
