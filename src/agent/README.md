...
- `freshExternalDataGuard.ts`: detects time-sensitive price, fare, schedule, availability, current sports-roster/transaction, and similar answers that lack fresh web evidence, drives one web-search-only retrieval retry, and fails closed instead of publishing invented live data.
