...
`src/agent/nanocodexRuntime.ts` owns the versioned process protocol. Request IDs, protocol versions, and terminal results are validated. Application control traffic uses a dedicated inherited pipe (fd 3), while process stdin stays disconnected so the embedded runtime and its child processes cannot consume protocol messages. The native process clears the inherited descriptor's nonblocking flag before its dedicated reader waits for later tool results. Tool-result writes wait until that pipe has flushed, and the native runtime acknowledges acceptance immediately in the canonical runtime ledger. Malformed or closed input fails the run visibly. Aborts terminate the process, and late tool calls still meet the application's abort and authority gates. Opaque NanoCodex snapshots are stored losslessly and resumed only when the session, model, instructions, and tool contract are compatible.
