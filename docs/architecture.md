...
| Unified improvement lifecycle | `improvementRepository.ts`, `improvementWorkRepository.ts`, `improvementVerificationRepository.ts`, `improvementProofProducerRepository.ts`, and `src/improvements/` |
...
| Improvement automation, detector authority, proof routing, producer liveness, and receipts | `src/improvements/reconciler.ts`, `detectorPolicies.ts`, `proofProducerRegistry.ts`, `proofAdapters.ts`, `verification.ts`, and the focused improvement repositories | detector-policy, reconciler, improvement verification, proof-producer, queue, workflow, and DB integration tests |
