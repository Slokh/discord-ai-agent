...
`src/agent/nanocodexRuntime.ts` owns the versioned process protocol. Request IDs, protocol versions, and terminal results are validated. Application control traffic enters on inherited fd 3, while process stdin stays disconnected so the embedded runtime and its child processes cannot consume protocol messages. Before NanoCodex starts, the native process moves that socket to a private high-numbered close-on-exec descriptor, closes fd 3, and clears the inherited nonblocking flag. Tool-result writes wait until the pipe has flushed, and the native runtime acknowledges acceptance immediately in the canonical runtime ledger. Any reader exit or panic is a terminal protocol error; malformed or closed input therefore fails the run visibly. Aborts terminate the process, and late tool calls still meet the application's abort and authority gates. Opaque NanoCodex snapshots are stored losslessly and resumed only when the session, model, instructions, and tool contract are compatible.
