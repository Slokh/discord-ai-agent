...
async function enqueueInteractionTurn(
  ...
  const runtime = await ensureAgentRuntimePromptExecution({
    ...
    status: "queued", source: `discord.${requestKind}`, qualityCohort: "member", executorName: input.agentExecutor?.name ?? "nanocodex", appRevision: input.config.appRevision, config: input.config,
  });
