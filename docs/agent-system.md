...
The `reminders` capability uses that grounded time context to translate conversational dates into an explicit RFC 3339 first instant and, when requested, a local wall-clock recurrence. `setReminder` owns creation, cancellation, pause, resume, and atomic edits through one mutation contract; `listMyReminders` remains a separate read. Both derive ownership from the immutable requester and current guild rather than model arguments. An omitted reminder ID resolves only from the exact direct-parent delivery message in the current channel. Recurring time edits require explicit `series` or `next_occurrence` scope, and a series replacement must provide its complete recurrence plus matching first occurrence. Reminder persistence, recurrence calculation, and delivery live outside the generic agent loop.
