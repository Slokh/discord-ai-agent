...
- `imageGenerationGuard.ts`: retries one explicit, unfulfilled image create/edit request by adding the image tool group and forcing `generateImage`; terse follow-ups are accepted only when current, reply-chain, or recent channel context establishes an image-generation thread.
