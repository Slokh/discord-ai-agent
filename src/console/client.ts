...
const detailSpecialLinks=(detail)=>detail?.release?.comparisonUrl?safeLink(detail.release.comparisonUrl,"Compare changes ↗"):"";
...
const traceItems=(story,detail)=>{
  ...
  for(const event of events){const metadata=event.metadata||{};const identity=metadata.toolName||metadata.model||event.title||"";items.push({id:event.id,key:"event:"+(event.code||event.id)+":"+event.level+":"+identity,type:event.type||"event",title:event.title||event.summary||event.code,summary:event.summary||"",status:event.status,level:event.level||"info",code:event.code||"",occurredAt:event.occurredAt,durationMs:event.durationMs,metadata,spanId:event.spanId,parentSpanId:event.parentSpanId,count:1,kind:"event",important:["model","tool","context","delivery","response"].includes(event.type)||["error","warn"].includes(event.level)||/(failed|failure|blocked|stalled|retry|queued|completed|delivered)/i.test(event.code||"")})}
  for(const check of list(detail?.release?.checks)){const failed=check.status==="failed";items.push({id:"release-check:"+check.name,key:"release-check:"+check.name,type:"check",title:human(check.name),summary:check.outcomeCode?human(check.outcomeCode):failed?"Verification failed":"Verification passed",status:failed?"failed":check.status==="started"?"running":"done",level:failed?"error":"info",code:"",occurredAt:check.completedAt||check.startedAt,durationMs:check.durationMs,metadata:{},count:1,kind:"release-check",important:true})}
  for(const run of list(detail?.embedding?.runs)){const failed=run.status==="failed";items.push({id:"embedding-run:"+run.executionId,key:"embedding-run:"+run.executionId,type:"batch",title:run.title||"Embedding batch",summary:(run.eventCount?run.eventCount+" events":"")+(run.warningCount?" · "+run.warningCount+" warnings":""),status:failed?"failed":"done",level:failed?"error":"info",code:"",occurredAt:run.completedAt||run.startedAt,durationMs:run.durationMs,metadata:{messageCount:run.messageCount},count:1,kind:"embedding-run",important:true})}
  const runs=detail?.embedding?[]:list(story.runs);for(const [index,run] of runs.entries())items.push({id:"run:"+run.id,key:"run:"+run.id,type:"run",title:run.title,summary:String(run.status).replaceAll("_"," "),status:run.tone==="danger"?"failed":"done",level:traceLevel(run.tone),occurredAt:run.occurredAt,durationMs:run.durationMs,count:1,kind:"run",important:index<3||run.tone==="danger"||run.tone==="warning"});
...
const embeddingModels=(detail)=>{const models=list(detail?.embedding?.models);if(!models.length)return "";return '<section class="detail-section detail-inventory"><div class="detail-section-heading"><h2>Index inventory</h2><span>'+esc(models.reduce((total,item)=>total+Number(item.count||0),0).toLocaleString())+' vectors</span></div><div class="inventory-list">'+models.map((item)=>'<div><strong>'+esc(item.model)+'</strong><span>'+esc(Number(item.count||0).toLocaleString())+' vectors · '+esc(item.dimensions)+' dimensions · input v'+esc(item.inputVersion)+'</span><time datetime="'+esc(item.latestEmbeddedAt)+'">updated '+esc(ago(item.latestEmbeddedAt))+'</time></div>').join("")+'</div></section>'};
const detailMetrics=(story,detail,active)=>{const release=detail?.release;const embedding=detail?.embedding;if(release){const checks=list(release.checks);const passed=checks.filter((item)=>item.status==="succeeded").length;const announcements=release.announcements||{};const receipts=release.verificationReceipts||{};const receiptTotal=Object.values(receipts).reduce((total,value)=>total+Number(value||0),0);const receiptFailed=Number(receipts.failed||0)+Number(receipts.inconclusive||0);return [detailMetric("Verified",exactTime(release.verifiedAt)),detailMetric("Deployment",release.deploymentId),detailMetric("Previous",release.previous?short(release.previous.revision,10):"First release"),detailMetric("Release checks",checks.length?passed+"/"+checks.length+" passed":"No checks recorded"),detailMetric("Improvement checks",receiptTotal?Number(receipts.passed||0)+"/"+receiptTotal+" passed":"None required",receiptFailed?"danger":""),detailMetric("Announcements",Number(announcements.posted||0)+"/"+Number(announcements.total||0)+" posted",announcements.failed?"danger":"")].filter(Boolean)}if(embedding){const coverage=embedding.coverage||{};const runs=list(embedding.runs);const messages=runs.reduce((total,item)=>total+Number(item.messageCount||0),0);const percent=Number(coverage.eligible)>0?Math.round(Number(coverage.embedded||0)/Number(coverage.eligible)*1000)/10:0;return [detailMetric("Runs",runs.length),detailMetric("Messages · 24h",messages.toLocaleString()),detailMetric("Failed",runs.filter((item)=>item.status==="failed").length,runs.some((item)=>item.status==="failed")?"danger":""),detailMetric("Coverage",percent+"%"),detailMetric("Unembedded",Number(coverage.unembedded||0).toLocaleString(),coverage.unembedded?"slow":""),detailMetric("p95 latency",Number.isFinite(story.p95DurationMs)?elapsed(story.p95DurationMs):null)].filter(Boolean)}return [detailMetric(active?"Running":"Occurred",active?duration(story.startedAt):exactTime(story.occurredAt)),detailMetric("Duration",active?duration(story.startedAt):Number.isFinite(story.durationMs)?elapsed(story.durationMs):null,story.latencyTone),detailMetric("Attempts",story.attempts>1?story.attempts:null),detailMetric("Branch",story.branchName),detailMetric("Runs",story.runCount),detailMetric("Successful",story.successCount),detailMetric("Failed",story.failureCount,story.failureCount?"danger":""),detailMetric("p95 latency",Number.isFinite(story.p95DurationMs)?elapsed(story.p95DurationMs):null)].filter(Boolean)};
...
function renderActivityDetail(detail,route){
  ...
  el("activity-detail").innerHTML='<article class="detail-hero '+esc(story.tone)+'"><div class="detail-eyebrow"><span class="story-mark" aria-hidden="true"></span><span>'+esc(activityKind(story.kind,story.hasParent))+'</span><span>'+esc(detailContext(story))+'</span></div><div class="detail-title-row"><div><h1>'+esc(story.title)+'</h1><p>'+esc(story.summary||String(story.status).replaceAll("_"," "))+'</p></div>'+badge(story.status)+'</div>'+allLinks+(metrics?'<section class="detail-metrics metrics-'+esc(metricItems.length)+'" aria-label="Activity facts">'+metrics+'</section>':"")+'</article>'+embeddingModels(detail)+detailTrace(story,detail);
