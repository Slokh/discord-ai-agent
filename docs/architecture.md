...
7. `src/discord/taskNotifications.ts` edits the original Discord status message with current progress and final PR/failure details, preferring `agent.task.*` events from the generic runtime session and falling back to legacy task events while the sandbox callback path still exists. The model-facing task-status tool follows the same source order for specific code-update tasks.
