...
2. Store `github_actions_candidate_role_arn` as GitHub secret `AWS_CANDIDATE_ROLE_ARN`.
   This role can publish only to tree-addressed candidate repositories, can be assumed only through the candidate workflow pinned to `main`, and has no EKS or production-repository access.
