...
4. `src/agent/router.ts` sends the user request, scoped conversation context, image context, skills, and tool schemas to the model.
   The first chat message is a compact, stable product/safety prompt so provider prefix caching can reuse it; tool-specific routing stays in scoped tool contracts. Requester identity, loaded skills, overlays, reply context, attachment guidance, and current-turn reminders remain in the initial contiguous system block before user/assistant session history, which preserves Claude-compatible role ordering. Top-level channel mentions can use the ordinary eight-message session window. Explicit replies omit channel-wide session memory and use only the retained reply chain plus the current turn, including when a queued turn starts after newer unrelated messages were posted. Prior-turn tool bodies are replaced with a pointer to retrieve or rerun the evidence when needed. `src/models/openrouter.ts` leaves implicit-cache providers alone and adds an Anthropic-only `cache_control` marker to that first system message; `runs:inspect` surfaces `cached_input` from provider usage metadata.
   Normal Discord conversation and tool selection use the configured primary chat model and reasoning effort with a bounded completion budget. Malformed tool calls, empty responses, leaked hosted-tool markup, repeated-tool termination, and provider-specific 400/422 request rejection use the configured recovery model for one bounded synthesis or retry; a rejected primary call keeps the remainder of that turn on recovery. Code-update tasks keep their independently configured codegen model.
