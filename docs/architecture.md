...
| Unified improvement lifecycle and automation liveness | `improvementRepository.ts`, `improvementWorkRepository.ts`, `improvementVerificationRepository.ts`, `improvementProofProducerRepository.ts`, and `src/improvements/` |
...
| Improvement automation, detector authority, proof routing, mutually observed producer liveness, and receipts | `src/improvements/reconciler.ts`, `producerHealth.ts`, `detectorPolicies.ts`, `proofProducerRegistry.ts`, `proofAdapters.ts`, `verification.ts`, and the focused improvement repositories | detector-policy, reconciler, producer-health, improvement verification, queue, workflow, and DB integration tests |
