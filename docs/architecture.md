...
| Improvement automation, detector authority, proof routing, mutually observed producer liveness, receipts, and bot alerts | `src/improvements/reconciler.ts`, `producerHealth.ts`, `detectorPolicies.ts`, `proofProducerRegistry.ts`, `proofAdapters.ts`, `verification.ts`, `src/discord/improvementBotUpdates.ts`, and the focused improvement repositories | detector-policy, reconciler, producer-health, bot-update, improvement verification, queue, workflow, and DB integration tests |
