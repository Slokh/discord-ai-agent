...
- Every Discord request runs a deterministic starter-funds preflight before model/tool selection. It tops the requester's verified balance up to the guild's durable starter target, falling back to the configured `$1` default, so tiny dust balances cannot block play and users never need special refill wording. Balances already at or above the target are untouched. Concurrent requests after the same below-target observation are rejected by the existing guarded second balance check and transfer lock. `requestStarterFunds` remains available as a safe fallback/recheck path.
