...
- Unicode `🐛` reaction add/remove events persist a requester-owned bug marker through `bugMarkerReaction.ts`; removing the reaction, emoji, or message clears the marker. On an AI reply, the first marker per deployed revision also creates a durable report and queues `bugReportAutomation.ts` without an extra intake model call. The task uses bounded run evidence. Confirmed fixes require a regression result, open a PR with private Discord evidence omitted, enable auto-merge behind required checks, and deploy through the normal main-branch workflow. Unconfirmed reports leave code unchanged. Every member uses the same flow; OpenRouter account credits are the spend ceiling.
