...
Discord task progress rendering now reads those mirrored `agent.task.*` runtime events first and falls back to legacy `task_events` only when a runtime mirror is not available. That makes the user-facing delivery path follow the replayable session event stream without requiring the sandbox runner callback protocol to be removed in the same migration slice.
