...
- `imageTools.ts`: Discord image inspection, scoped reference image collection, image generation, and generated-file conversion. Current/reply image references stay attached to a generation turn unless the current user explicitly opts out; a model-only `useContextImages: false` cannot silently discard them. A provider request-rejection gets one bounded recovery-prompt retry inside the tool so successful recovery attaches the image without depending on another model-selected tool call.
