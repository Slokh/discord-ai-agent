...
The canonical tables are `improvement_cases`, `improvement_signals`, `improvement_evidence`, `improvement_contracts`, and `improvement_case_events`. `src/db/improvementRepository.ts` is their only application data owner. `agent_tasks.improvement_case_id` links explicitly authorized code work without turning the task projection into a second case tracker.
...
- `resolved` requires supporting `deployment_verification` evidence and is written atomically with that evidence.
...
npm run improve -- --target local verify <case-id> --revision <sha> --summary "..."
```
