...
- Image edits inline bounded, permission-scoped Discord CDN references before calling the generation provider so private or signed attachment URLs do not depend on provider-side downloads. A text-only request blocked by the image provider gets one conservative, clearly stylized safety retry. A blocked transparent reference edit gets one background-only recovery pass with the same scoped references; other reference edits and a second safety rejection continue to fail closed.
...
`registry.ts` assigns each model-facing tool to a coarse `group`: `core`, `discord-retrieval`, `generated-data`, `presentation`, `discord-action`, `image`, `spotify`, `codegen`, `ops`, or `external`. `toolScope.ts` deterministically selects groups per turn. Ordinary chat begins with the two core escalation/help tools and hosted web/date tools. The `requestAdditionalTools` schema provides a concise capability map for the groups; detailed routing instructions arrive only with the scoped tools themselves. The model loop retains `drawRandom` only when the current request, retained reply chain, active game action, or loaded prompt context establishes chance intent; a terse continuation of an earlier draw still works, while an unrelated follow-up cannot spend a round or consume entropy through a speculative model choice. Explicit requests to perform a chance action force `drawRandom`; all-in wallet wagers first force a verified balance read and force `drawRandom` on the following round. Questions about odds and game rules remain conversational. Explicit Unicode/custom-emoji reaction requests preload `addDiscordReaction`; the action accepts only exact visible message targets, validates current-server custom emoji, and requires either explicit reaction wording or the exact target URL in the current mutation-authorized message. When `USER_WALLETS_ENABLED=false`, deployment scoping removes user-wallet actions and the optional wager field from `drawRandom`; bot balance reads remain available when the shared wallet is enabled. Rich-presentation schemas are selected for explicit UI requests or requested by the model through the escalation tool when gathered evidence would benefit from native Discord UI. Other Discord action schemas, retrieval, and generated-data tools are added only when the request indicates those capabilities.

Scoping is controlled by `TOOLSET_SCOPING` (default `true`). When wallet-backed wagers are enabled, `awaitRandomWagerAction` and `settleRandomWager` are always paired with `drawRandom` so a reserved wager can either pause durably for a genuine player decision or settle immediately when the verified draw is terminal. A new wallet wager must declare the current requester's Discord user id; the tool validates it against the immutable ingress scope and rejects mentioned, replied-to, or other third-party players before reserving funds or drawing. Continuation tools resolve the canonical wager from requester and Discord game-session scope instead of exposing or trusting model-copied database ids. Interactive state is generic, bounded JSON with optimistic versions and allowed actions; replies resume it only for the original player in the same Discord chain. When a long reply chain reaches the prompt-context cap, active-game lookup also matches the wager's opening or latest-action message against the retained chain while preserving the requester and channel-key prefix. Confirmation and settlement are not accepted as fake player actions. Settlement is ledger-validated: payout direction must match the structured outcome; a draw-determined terminal outcome may settle in the opening request, while a player-decision settlement requires persisted state plus a later Discord reply. Real-money wagers based on a player's uncommitted secret are rejected before reservation or randomness. Machine-recognizable coin, dice, and bounded-integer contracts are probability-checked against their maximum payout, rejecting guaranteed-profit and negative-EV games before any reservation or draw. Unsupported custom profit conditions fail closed across draw kinds instead of relying on the model to estimate their fairness. A bare current-request amount is authoritative, and the payment ledger permits only one wager reservation per Discord request and one active game per player/reply chain so parallel model tool calls cannot double-reserve funds. Ordinary requester transfers require explicit transfer language in the current prompt. Spotify tools are deployment-gated and only appear when `SPOTIFY_CLIENT_ID` and `SPOTIFY_CLIENT_SECRET` are configured; codegen tools are similarly hidden unless a real GitHub repository is configured. The hidden `requestAdditionalTools` core tool is an escalation valve: if the model notices a missing capability, it can request specific groups (or all groups), and the next model round in the same turn is recomputed with the expanded toolset. Its schema enumerates the valid group names. An invalid group request expands all deployment-available groups and reports the invalid names instead of silently leaving the model without the capability it needs.
