...
Known source-owned gates may propose an executable contract. Semantic assessment may propose only registered tool or answer-text checks, or explicitly select an observational detector's already-registered recovery contract; invalid, invented, or non-executable contracts cannot unlock repair. Unknown automated detector codes still await a registered source-owned contract.
