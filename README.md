...
| `AGENT_RUNTIME_EXECUTION_BACKEND` | `in-process` | Prompt execution backend; `warm-sandbox` runs the Discord prompt through the serialized warm-runtime envelope |
| `AGENT_RUNTIME_WARM_SANDBOX_URL` | unset | Optional warm prompt server URL, for example `http://discord-ai-agent-agent-runtime:8090`; when unset, `warm-sandbox` falls back to the local child runner |
| `AGENT_RUNTIME_WARM_SANDBOX_HOST` / `AGENT_RUNTIME_WARM_SANDBOX_PORT` | `0.0.0.0` / `8090` | Bind address for `npm run agent:warm-server` / `npm run start:agent-warm-server` |
...
| `CONTROL_UI_PUBLIC_URL` | unset | Public console origin, for example `https://tasks.example.com`; when set, Discord code-update progress messages include a run-console link |
