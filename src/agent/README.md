...
- `modelTimeoutFallback.ts`: trims oldest conversational history for the one safe utility-model retry, preserves a newly expanded tool choice by retrying it with the utility model, and otherwise performs tool-free utility-model synthesis when the primary model times out after tools have gathered evidence.
