...
- `walletTools.ts`: conversational shared-wallet lifecycle status, roster-independent funded-wallet directories backed by existing wallet accounts, requester-scoped canonical wager history, safe Discord-name resolution with one live roster fetch per turn and a permission-filtered indexed fallback, below-target starter top-ups, requester transfers including explicit whole-balance sends, and authorized reconciliation.
