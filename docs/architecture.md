...
4. Request preflight loads the server's durable primary chat-model override. An exact owner/ops request such as `@ai switch model to moonshotai/kimi-k3` or `@ai reset model` is handled deterministically before model selection, so a broken override cannot block its own recovery. Otherwise, `src/agent/router.ts` sends the user request, scoped conversation context, image context, skills, and tool schemas to the model.
