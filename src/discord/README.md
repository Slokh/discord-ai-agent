...
- Unicode `🐛` reaction add/remove events persist a requester-owned bug marker through `bugMarkerReaction.ts`; removing the reaction, emoji, or message clears the marker. On an AI reply, the first marker per deployed revision also creates a durable report and queues `bugReportAutomation.ts` without an extra intake model call or Discord status reply. The task uses bounded run evidence. Confirmed fixes require a regression result, open a PR with private Discord evidence omitted, enable auto-merge behind required checks, and deploy through the normal main-branch workflow. On bot readiness, `deployedBugRetry.ts` verifies through GitHub that the exact merge is contained in the running revision, claims the report once, silently reruns the original requester-scoped prompt, and replaces the marked reply only with the final corrected result. Unconfirmed or failed reports emit one terminal reply and leave code unchanged. Every member uses the same flow; OpenRouter account credits are the spend ceiling.
...
- Bug-marker reactions and automation: `tests/unit/bug-marker-reaction.test.ts` and `tests/unit/bug-report-automation.test.ts`.
