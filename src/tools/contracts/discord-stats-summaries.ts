...
export const discordStatsSummaryToolContracts = [
  defineTool({
    ...
    examples: [
      ...
      "@ai can you check indexed word counts as well as message totals?",
    ],
    description:
      "Compute permission-filtered Discord analytics over indexed visible messages. ALWAYS call this before answering a request or capability question about indexed Discord message, character, or word counts; return an authoritative result instead of claiming those totals are unavailable, estimating from retrieved snippets, or merely describing the capability. After a successful call, include the result's exact `Metric: <label>` line verbatim in the final reply so it is explicit what was counted. If a capability question names multiple supported metrics, run one requested representative metric, preferring words when words are named, and state that the other named metrics are also supported. Use this for rankings; lowest/highest rankings; activity over time; messages by channel/user; normalized messages-per-day comparisons; attachment stats; reaction totals; and active-day summaries. Character and word counts use stored normalized message text. Time buckets use UTC. Hour-of-day and day-of-week results describe observed message timing only; they do not establish sleep, location, work schedule, or availability. Keep a simple named-person activity follow-up to the requested peak or comparison instead of dumping every bucket.",
    mutates: false,
