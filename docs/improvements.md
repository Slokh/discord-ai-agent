...
A signal is an observation, never a defect verdict. Reports and observational operational incidents authorize autonomous assessment: assessment keeps the checkout clean, and only a confirmed machine-executable contract unlocks edits. Deterministic CI, deploy, eval, and runtime gates retain direct authority when their stable code maps to a registered proof adapter. Schedule-health observations do not: the assessor must dismiss transient or expected behavior, request a specific affected-member action or clarification, or confirm a product defect before code work begins. `🐛` is visible to members who can see the source channel; it creates a private durable member signal, and removing it withdraws that signal. The model may silently record generalized internal impediments, but it must still answer the member normally.
