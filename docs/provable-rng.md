...
- **The model supplies canonical draw parameters.** Ordinary wording is interpreted into the typed draw schema rather than reclassified by application regex. Standard blackjack validation still requires exactly the two player cards plus dealer upcard; typed `hit`/`stand` continuations draw only the newly public card data required by that durable action.
