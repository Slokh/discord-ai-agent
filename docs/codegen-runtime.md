...
Queue handoff bookkeeping is split by lifecycle phase. The queue creates initial task-linked execution rows before `pg-boss` enqueue so a fast worker can mark them running, then only attaches the returned pg-boss job id as metadata. Do not re-create those executions with `status: queued` after enqueue; that can race with the worker and make an active runtime look queued again.
