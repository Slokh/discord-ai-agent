...
- All bot-owned updates share one configured Discord channel: deployment announcements, automation-health alerts, improvement questions, repair progress, verification, and fixes. Ordinary request replies and their task status remain in the member's originating conversation.
...
Operators have one private, read-only dashboard for the system's current shape: service liveness, prompts, repository work, improvements, proof producers, verified releases, and typed activity. It projects the canonical runtime and improvement records instead of introducing a parallel tracker. One compact top bar holds environment, revision, freshness, services, proof producers, releases, and attention, with secondary detail available on hover or keyboard focus. Below it, the Console is a master-detail workspace: a fixed Activity sidebar keeps filters and the independently scrollable story index visible while the selected story's context and execution detail update in place beside it. Current prompts and code work are pinned first, every non-terminal improvement remains present even when its latest event is outside the recent window, linked repair work folds into the same improvement story, and repeated successful system jobs collapse into rollups. All, Active, Waiting, Blocked, Failures, and System filters expose the relevant operational slice without creating separate queues; the top-bar attention control opens Blocked directly. Transient service and proof health remains in the top bar, while sustained actionable failures enter the improvement lifecycle. Synthetic prompt executions and post-deploy canary tasks remain in their canonical ledgers and proof paths but do not enter Activity. Prompt outcomes distinguish response generation from authoritative Discord delivery. Reply-originated rows use Reply as their sole type label, while top-level rows use Prompt; parent content is not duplicated in the overview. Each activity row keeps type, recency, duration, and high-signal labels or exceptional outcome text on the first line, then reserves the second line for the title or prompt. Selecting a row updates the URL and browser history without reloading either pane, preserving deep links to the type-appropriate facts, lifecycle, source/reply/thread or pull-request links, individual background runs, and technical events. On narrow screens the same interaction becomes an accessible list-to-detail transition with a back control. Prompt details additionally join the retained Discord ancestor chain with the execution-scoped current prompt and final runtime reply, fetched only when that detail is selected. This Context view mirrors Discord's message hierarchy, renders retained text and attachment metadata, labels deleted messages, and uses each timestamp as the link back to Discord. It initially shows only the direct reply parent, current prompt, and final reply; operators can expand the earlier ancestor chain when deeper context is useful. The overview shows a bounded prompt preview because it runs inside the trusted cluster boundary; it omits member identity, full transcripts, artifacts, secret-bearing metadata, and private model reasoning. The console has no public ingress and no mutation endpoints.
