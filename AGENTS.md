...
- Agent-runtime backend selection and prompt executor behavior live in `src/agent/runtimeRunner.ts` and `src/agent/runtimeExecutor.ts`; the out-of-process prompt runner is `src/agent/sandboxPromptRunner.ts`.
