...
- `toolScope.ts`: publishes a compact deployment-aware capability index and starts ordinary turns with only discovery/external tools. The model requests exactly the needed group; attachment presence may pre-enable file/image inspection because it is structural input, not inferred intent.
...
- `imageTools.ts`: Discord image inspection, scoped reference image collection, image generation, and generated-file conversion. Current/reply image references stay attached to a generation turn unless the current user explicitly opts out; a model-only `useContextImages: false` cannot silently discard them. A provider request-rejection gets one bounded compatibility-prompt retry inside the tool so successful recovery attaches the image without depending on another model-selected tool call.
- `imageContextSelection.ts`: current-turn opt-out policy for whether generated-image reply references remain attached.
...
- `standardWagerRuntime.ts`: loads scoped RNG evidence, derives whose standard-blackjack card comes next, and applies the deterministic settlement result at the money-moving tool boundary.
...
- `agentModelIntent.ts`, `agentModelCatalog.ts`, and `agentModelTools.ts`: owner/ops-only durable per-server primary-model switching and reset. Explicit natural requests such as `@ai switch to Sonnet 5`, compact aliases, exact OpenRouter IDs, typo-tolerant switch verbs, contextual `switch to that`, and compound switch-and-work requests bypass the current model. Aliases are resolved through a bounded cached OpenRouter catalog; the current message remains authoritative when a model-generated tool argument conflicts. Successful changes apply to remaining same-request work and future requests while leaving recovery-model configuration unchanged.
...
`registry.ts` assigns each model-facing tool to a coarse `group`: `core`, `discord-retrieval`, `generated-data`, `presentation`, `discord-action`, `image`, `spotify`, `codegen`, `ops`, or `external`. With `TOOLSET_SCOPING=true`, ordinary chat begins with core discovery/skill-loading and hosted external tools; a compact deployment-aware capability index tells the model what can be loaded. The model calls `requestAdditionalTools` for exactly the group it needs, and the next model round receives both the matching schemas and concise operational guidance. File/image attachment presence may pre-enable inspection because it is a structural input fact; normal-language keywords, reply text, and model-switch wording do not select tool groups. An intentional all-groups request remains available, but invalid group names do not silently expand scope. Tool contracts are the canonical detailed interface.

Active games structurally enable the `discord-action` group so their continuation tools are present. New chance requests are model-led: the model loads `discord-action`, then `drawRandom` independently verifies an explicit current-turn chance authorization before creating a session, consuming entropy, or reserving money. A new wallet wager declares the immutable requester and may provide `interactionMode` plus a structured rule (`coin_side`, `sum`, `any_match`, or `all_distinct`). Fairness and payout limits are calculated from that rule in code; legacy prose recognition is only a compatibility fallback. Continuation tools resolve the canonical wager from requester and Discord game-session scope rather than trusting model-copied ids. Settlement remains ledger-validated, and standard coin-flip/blackjack results are recomputed from persisted verified draws before money moves. Replies resume games only for the original player in the same Discord chain. A bare current-request amount is authoritative, the payment ledger permits one reservation per request and one active game per player/reply chain, and ordinary transfers require explicit current-turn language. Deployment gates still hide Spotify/codegen and wallet features when not configured.
